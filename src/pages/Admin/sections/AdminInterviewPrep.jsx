...
export default function AdminInterviewPrep() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data: cats }, { data: topics }] = await Promise.all([
      supabase.from('interview_categories').select('*'),
      supabase.from('interview_topics').select('*'),
    ]);
  ...
  const saveCat = async (e) => {
    ...
    let error;
    if (editCatId) {
      ({ error } = await supabase.from('interview_categories').update({ label: catForm.label, icon: catForm.icon, color: catForm.color }).eq('id', editCatId));
    } else {
      ({ error } = await supabase.from('interview_categories').insert([catForm]));
    }
    setSaving(false);
    if (error) showToast(error.message, 'error');
    else { showToast(editCatId ? 'Category updated!' : 'Category added!'); setShowCatForm(false); loadData(); }
  };
  ...
  const deleteCat = async (id) => {
    ...
    if (error) showToast(error.message, 'error');
    else { showToast('Category deleted'); loadData(); }
  };
  ...
  const saveTopic = async (e) => {
    e.preventDefault();
    setSaving(true);
    const payload = { ...topicForm, category_id: topicCatId };
    let error;
    if (editTopicId) {
      ({ error } = await supabase.from('interview_topics').update(payload).eq('id', editTopicId));
    } else {
      ({ error } = await supabase.from('interview_topics').insert([payload]));
    }
    setSaving(false);
    if (error) showToast(error.message, 'error');
    else { showToast(editTopicId ? 'Topic updated!' : 'Topic added!'); setShowTopicForm(false); loadData(); }
  };
  ...
  const deleteTopic = async (id) => {
    ...
    if (error) showToast(error.message, 'error');
    else { showToast('Topic deleted'); loadData(); }
  };
