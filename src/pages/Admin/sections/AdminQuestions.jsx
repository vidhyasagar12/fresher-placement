...
export default function AdminQuestions() {
  ...
  const handleSave = async (e) => {
    ...
    let error;
    if (editId) {
      ({ error } = await supabase.from('interview_questions').update(form).eq('id', editId));
    } else {
      ({ error } = await supabase.from('interview_questions').insert([form]));
    }
    setSaving(false);
    if (error) showToast(error.message, 'error');
    else { showToast(editId ? 'Question updated!' : 'Question added!'); setShowForm(false); loadQuestions(); }
  };
